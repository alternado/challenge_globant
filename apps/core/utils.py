    ...
    :param request: Django HTTP request containing datatables parameters: `length`, `start` and `draw`.
          `length`: Number of records that the table can display in the current draw.
           `start`: This is the start point in the current data set 0-indexed.
            `draw`: Draw counter. This is used by DataTables to ensure that the Ajax returns from server-side processing
...
def datatable_page_from_list(tabla, request):
    """
    Returns queryset paginated with datatables parameters
    ...
    try:
        ...
        length = 10
    if length == -1:
        length = len(tabla) or 10
    paginator = Paginator(tabla, length)
    try:
        start = int(request.GET.get('start', 0))
    except ValueError:
        start = 0
    ...
    try:
        ...
        draw = 0
    tabla = tabla[page.start_index() - 1: page.end_index()]
    return tabla, draw, page
